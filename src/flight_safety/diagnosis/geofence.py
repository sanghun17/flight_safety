...
INSIDE -> OK | APPROACHING -> WARN | OUTSIDE -> ERROR.  A stale VRPN pose is WARN
while MAVROS local_position is fresh, and ERROR only when both are unavailable.
...
class GeofenceDiag(object):
    def __init__(self, cfg):
        ...
        rospy.Subscriber(cfg["fallback_source"], PoseStamped,
                         self._on_fallback_pose, queue_size=5)
    ...
    def _on_fallback_pose(self, _m):
        self.last_fallback_rx = rospy.Time.now()

    @staticmethod
    def _fresh(last_rx, now, timeout):
        return last_rx is not None and (now - last_rx).to_sec() <= timeout

    def fallback_fresh(self, now):
        return self._fresh(self.last_fallback_rx, now, self.fallback_timeout)

    def status(self, now):
        # returns (state, margin_m): signed min distance to nearest wall, <0 once outside.
        if not self._fresh(self.last_rx, now, self.timeout):
            return UNKNOWN, None
        margin = float("inf")
    ...
    def run_diag(self, stat):
        ...
        if st == OUTSIDE:
            ...
            if self.fallback_fresh(now):
                stat.summary(
                    DiagnosticStatus.WARN,
                    "no VRPN pose; local_position alive (controlled LAND)")
            else:
                stat.summary(
                    DiagnosticStatus.ERROR,
                    "no VRPN pose and no local_position (KILL)")
        else:
