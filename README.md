...
**control 우선순위 사다리** (supervisor `_tick`에 명시, **Manual이 최상단**):
- **Manual (최상단)**: 비-OFFBOARD 모드거나 OFFBOARD 중 RC 스틱 편향 → 파일럿이 차량 소유. **이때 companion은 아무 것도 안 함 (kill 포함 X)**. OFFBOARD+RC편향이면 POSCTL로 이양.
- **KILL**: (자율/OFFBOARD 중에만) terminal(geofence 이탈 등) → force-disarm
- **Response**: 활성 응답(hold/land) setpoint
- **Normal**: `normal_in`(JAX/control_bridge) passthrough
- Response/Normal은 출력 1곳에서 geofence clamp 적용 → `setpoint_out`

✅ 통합됨(2026-06-24): control_bridge가 `/local_controller/setpoint_raw/local`로 발행 → MUX(`normal_in`)가 staleness 가드 후 `setpoint_out`으로 forward(disarmed에서도 = OFFBOARD 진입 스트림 유지). setpoint frame/sign 해소: FRAME_BODY_NED raw passthrough + yaw_rate 제어.
